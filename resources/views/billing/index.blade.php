...
<div class="min-h-screen bg-[#0a0a0a] p-4 md:p-8">
    <div class="w-full mx-auto space-y-6">
        ...
        <div class="card-dark p-4">
            <form method="GET" action="{{ route('billing.index') }}" class="flex gap-3">
                <div class="flex-1 relative">
                    <input type="text" name="search" value="{{ request('search') }}" placeholder="Search by member name, amount, or status..." class="input-dark w-full pl-10 pr-4 py-2.5">
                ...
                @if(request('search'))
                    <a href="{{ route('billing.index') }}" class="btn-secondary px-6">Clear</a>
                @endif
            </form>
        </div>
        ...
        <div class="card-dark overflow-hidden">
                                ...
                                <td class="px-6 py-4 whitespace-nowrap">
                                    <div class="flex items-center">
                                        <div class="w-10 h-10 rounded-full flex items-center justify-center mr-3" style="background: linear-gradient(135deg, #ADFF2F 0%, #7CB518 100%);">
                                            <span class="text-black font-bold text-sm">{{ substr($billing->member && $billing->member->user ? $billing->member->user->nama : 'M', 0, 1) }}</span>
                                        </div>
                                        <div>
                                            <div class="text-sm font-semibold text-white">{{ $billing->member && $billing->member->user ? $billing->member->user->nama : '-' }}</div>
                                        </div>
                                    </div>
                                </td>
                                <td class="px-6 py-4 whitespace-nowrap">
                                    <span class="inline-flex items-center px-3 py-1 rounded-full text-xs font-medium bg-neutral-700 text-neutral-300">
                                        {{ $billing->membershipPlan ? $billing->membershipPlan->nama_plan : '-' }}
                                    </span>
                                </td>
                                <td class="px-6 py-4 whitespace-nowrap">
                                    <div class="text-sm font-semibold text-[#ADFF2F]">Rp {{ number_format($billing->jumlah, 0, ',', '.') }}</div>
                                </td>
                                <td class="px-6 py-4 whitespace-nowrap">
                                    <div class="text-sm text-neutral-300">
                                        <div class="flex items-center">
                                            ...
                                            <div>
                                                <div class="text-xs text-neutral-500">Billing: {{ $billing->tanggal_tagihan }}</div>
                                                <div>Due: {{ $billing->tanggal_jatuh_tempo }}</div>
                                            </div>
                                        </div>
                                    </div>
                                </td>
                                <td class="px-6 py-4 whitespace-nowrap">
                                    @if($billing->status_pembayaran == 'Lunas')
                                        <span class="inline-flex items-center px-3 py-1 rounded-full text-xs font-semibold bg-[#ADFF2F] text-black">
                                    ...
                                    @else
                                        <span class="inline-flex items-center px-3 py-1 rounded-full text-xs font-semibold bg-red-600/20 text-red-400">
                                            <svg class="w-3 h-3 mr-1" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                ...
                                <td class="px-6 py-4 whitespace-nowrap text-sm font-medium">
                                    @if($billing->billing_id)
                                        <div class="flex space-x-2">
                                            <a href="{{ route('billing.show', $billing->billing_id) }}"
                                            ...
                                            <a href="{{ route('billing.edit', $billing->billing_id) }}"
                                            ...
                                            <form action="{{ route('billing.destroy', $billing->billing_id) }}" method="POST" class="inline" onsubmit="return confirm('Yakin hapus pembayaran?')">
                                                @csrf
                                                @method('DELETE')
                                                <button type="submit"
            ...
            @if($billings->hasPages())
                <div class="px-6 py-4">
                    <x-pagination :paginator="$billings" />
                </div>
            @endif
        </div>
    </div>
