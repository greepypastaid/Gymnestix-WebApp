    ...
    <div class="w-full mx-auto">
        ...
        <div class="card-header mb-6">
            <div class="flex items-center gap-3">
                ...
                <div>
                    <div class="title">Edit Billing</div>
                    <div class="subtitle">Update billing information</div>
                </div>
            </div>
            <div class="ml-auto">
            ...
            <form action="{{ route('billing.update', $billing->billing_id) }}" method="POST" class="space-y-6">
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    <div>
                        ...
                        <select name="member_id" id="member_id" class="input-dark w-full" required disabled>
                    ...
                    <div>
                        <label for="jumlah" class="block text-gray-400 mb-2">Jumlah</label>
                        <input type="number" name="jumlah" id="jumlah" class="input-dark w-full" value="{{ old('jumlah', $billing->jumlah) }}" required readonly placeholder="Amount">
                    </div>
                    <div>
                        <label for="status_pembayaran" class="block text-gray-400 mb-2">Status Pembayaran</label>
                        <input type="text" name="status_pembayaran" id="status_pembayaran" class="input-dark w-full" value="{{ old('status_pembayaran', $billing->status_pembayaran) }}" required placeholder="e.g. Paid, Pending">
                    </div>
                    <div>
                        <label for="tanggal_tagihan" class="block text-gray-400 mb-2">Tanggal Tagihan</label>
                        <input type="date" name="tanggal_tagihan" id="tanggal_tagihan" class="input-dark w-full" value="{{ old('tanggal_tagihan', $billing->tanggal_tagihan ? \Carbon\Carbon::parse($billing->tanggal_tagihan)->format('Y-m-d') : '') }}" required>
                    </div>
                    <div>
                        <label for="tanggal_jatuh_tempo" class="block text-gray-400 mb-2">Tanggal Jatuh Tempo</label>
                        <input type="date" name="tanggal_jatuh_tempo" id="tanggal_jatuh_tempo" class="input-dark w-full" value="{{ old('tanggal_jatuh_tempo', $billing->tanggal_jatuh_tempo ? \Carbon\Carbon::parse($billing->tanggal_jatuh_tempo)->format('Y-m-d') : '') }}" required>
                    </div>
                </div>
