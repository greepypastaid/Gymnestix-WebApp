...
<div class="py-12 bg-black min-h-screen">
    <div class="max-w-4xl mx-auto sm:px-6 lg:px-8">
        ...
        <div class="bg-neutral-800 p-6 shadow sm:rounded-lg mb-6">
            <div class="flex items-center justify-between">
                <div class="flex items-center space-x-4">
                    ...
                    <div>
                        <h1 class="text-2xl font-bold text-white">Edit Billing</h1>
                        <p class="text-neutral-400">Update billing information</p>
                    </div>
                </div>
        ...
        <div class="bg-neutral-800 shadow sm:rounded-lg p-6">
            <form action="{{ route('billing.update', $billing->billing_id) }}" method="POST" class="space-y-6">
                @csrf
                @method('PUT')
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    <div>
                        <label for="member_id" class="block text-sm font-medium text-white mb-2">Member</label>
                        <select name="member_id" id="member_id" class="block w-full px-3 py-2 border border-neutral-600 rounded-lg shadow-sm bg-neutral-700 text-neutral-400 focus:outline-none focus:ring-2 focus:ring-[#ADFF2F] focus:border-[#ADFF2F]" required disabled>
                    ...
                    <div>
                        ...
                        <input type="number" name="jumlah" id="jumlah" class="block w-full px-3 py-2 border border-neutral-600 rounded-lg shadow-sm bg-neutral-700 text-neutral-400 focus:outline-none focus:ring-2 focus:ring-[#ADFF2F] focus:border-[#ADFF2F]" value="{{ old('jumlah', $billing->jumlah) }}" required readonly placeholder="Amount">
                    </div>
                    <div>
                        <label for="status_pembayaran" class="block text-sm font-medium text-white mb-2">Status Pembayaran</label>
                        <input type="text" name="status_pembayaran" id="status_pembayaran" class="block w-full px-3 py-2 border border-neutral-600 rounded-lg shadow-sm bg-neutral-700 text-white focus:outline-none focus:ring-2 focus:ring-[#ADFF2F] focus:border-[#ADFF2F]" value="{{ old('status_pembayaran', $billing->status_pembayaran) }}" required placeholder="e.g. Paid, Pending">
                    </div>
                    <div>
                        <label for="tanggal_tagihan" class="block text-sm font-medium text-white mb-2">Tanggal Tagihan</label>
                        <input type="date" name="tanggal_tagihan" id="tanggal_tagihan" class="block w-full px-3 py-2 border border-neutral-600 rounded-lg shadow-sm bg-neutral-700 text-white focus:outline-none focus:ring-2 focus:ring-[#ADFF2F] focus:border-[#ADFF2F]" value="{{ old('tanggal_tagihan', $billing->tanggal_tagihan ? \Carbon\Carbon::parse($billing->tanggal_tagihan)->format('Y-m-d') : '') }}" required>
                    </div>
                    <div>
                        <label for="tanggal_jatuh_tempo" class="block text-sm font-medium text-white mb-2">Tanggal Jatuh Tempo</label>
                        <input type="date" name="tanggal_jatuh_tempo" id="tanggal_jatuh_tempo" class="block w-full px-3 py-2 border border-neutral-600 rounded-lg shadow-sm bg-neutral-700 text-white focus:outline-none focus:ring-2 focus:ring-[#ADFF2F] focus:border-[#ADFF2F]" value="{{ old('tanggal_jatuh_tempo', $billing->tanggal_jatuh_tempo ? \Carbon\Carbon::parse($billing->tanggal_jatuh_tempo)->format('Y-m-d') : '') }}" required>
                    </div>
                </div>
