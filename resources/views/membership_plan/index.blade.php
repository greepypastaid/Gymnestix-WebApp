...
<div class="min-h-screen bg-[#0a0a0a] p-4 md:p-8">
    <div class="w-full mx-auto space-y-6">
        ...
        <div class="card-dark p-4">
            <form method="GET" action="{{ route('membership_plan.index') }}" class="flex gap-3">
                <div class="flex-1 relative">
                    <input type="text" name="search" value="{{ request('search') }}" placeholder="Search by plan name, description, or price..." class="input-dark w-full pl-10 pr-4 py-2.5">
                ...
                @if(request('search'))
                    <a href="{{ route('membership_plan.index') }}" class="btn-secondary px-6">Clear</a>
                @endif
            </form>
        </div>
