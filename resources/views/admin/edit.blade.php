...
<div class="py-6 sm:py-12 bg-black min-h-screen">
    <div class="max-w-3xl mx-auto px-4 sm:px-6 lg:px-8 space-y-4 sm:space-y-8">
        ...
        <div class="bg-neutral-800 p-4 sm:p-6 shadow sm:rounded-lg mb-6">
            <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
                <div class="flex items-center space-x-3 sm:space-x-4">
                    ...
                    <div>
                        <h1 class="text-xl sm:text-2xl font-bold text-white">Edit User</h1>
                        <p class="text-sm text-neutral-400 hidden sm:block">Update user information</p>
                    </div>
                </div>
        ...
        <div class="bg-neutral-800 shadow sm:rounded-lg p-4 sm:p-6">
            <form action="{{ route('admin.update', $user->user_id) }}" method="POST" class="space-y-4 sm:space-y-6">
                @csrf
                @method('PUT')
                <div class="grid grid-cols-1 md:grid-cols-2 gap-4 sm:gap-6">
                    <div>
                        <label for="nama" class="block text-xs sm:text-sm font-medium text-white mb-1.5 sm:mb-2">Nama</label>
                        <input type="text" name="nama" id="nama" class="block w-full px-3 py-2 text-sm border border-neutral-600 rounded-lg shadow-sm bg-neutral-700 text-white focus:outline-none focus:ring-2 focus:ring-[#ADFF2F] focus:border-[#ADFF2F]" value="{{ old('nama', $user->nama) }}" required placeholder="Enter full name">
                    </div>
                    <div>
